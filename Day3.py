...
dirSign = {"U":1,"D":-1,"L":-1,"R":1}
dirPlane = {"U":"y","D":"y","L":"x","R":"x"}
...
class Wire:
    ...
    def populate_paths(self):
        for path in self.PathList:
            direction = path[:1]
            distance = path[1:]
            self.MovePath.append(direction)
            self.DistPath.append(int(distance))
...
class Grid:
    ...
    def add_point(self, x, y, wire):
        currentPoint = self.get_point(x, y)
        if currentPoint == 0 or currentPoint == wire:
            point = wire
        else:
            point = 3
        self.Map[(x,y)] = point

    def add_path(self, wireMoveList, wireDistList, wireNum):
        currentX = 0
        currentY = 0

        for i in range(0, len(wireMoveList)):
            sign = dirSign[wireMoveList[i]]
            plane = dirPlane[wireMoveList[i]]

            for j in range(1, (wireDistList[i])+1):
                if plane == "y":
                    currentY += sign
                else:
                    currentX += sign
                self.add_point(currentX, currentY, wireNum)

    def add_steps(self, wireMoveList, wireDistList, wireNum):
        if wireNum == 1:
            intMap = self.StepMap1
        else:
            intMap = self.StepMap2
        currentX = 0
        currentY = 0
        steps = 0
...
def get_grid():
    ...
    grid.add_steps(wire1.MovePath, wire1.DistPath, 1)
    grid.add_steps(wire2.MovePath, wire2.DistPath, 2)

    steps = 9999999
    for cross in crosses:
        steps1 = grid.StepMap1[cross]
        steps2 = grid.StepMap2[cross]
        total = steps1 + steps2
...
def find_closest(crossList):
    distances = []
    
    for tup in crossList:
        x = abs(tup[0])
        y = abs(tup[1])
        dist = x + y
