...
#   if BBB)CCC - CCC is in orbit around BBB, and CCC is indirectly orbiting AAA
#   any number of objects, if CCC)DDD, DDD also indirectly orbits AAA
def get_orbits(oFile):
    orbits = {}
    objects = set()
...
with open("orbitTestData.txt") as oFile:
    orbits, objects = get_orbits(oFile)

print(f"{get_total_orbits(orbits)}")
print(f'{get_path_between("YOU", "SAN", orbits)}')
